...
int StackPush(Stack** stack, int value){//adding elements to the stack
           ...
           Stack * current;//a temparory variable
           current=(Stack *)malloc(sizeof(Stack));//making a new node
           current->next=*stack;//assigning the previous node to current nodes next
           current->value=value;
           
           stack=&current;//assigining the new top node
           
           return 0;
       }


int StackPop(Stack**stack, int* value){//popping elments from the stackptr

if(*stack==NULL){//underflow error
       return -1;}
else{
  *value=(*stack)->value;
...
node_t * temp;
 ...
 //current=queue->quetopptr;
 	...
 	while(temp!=NULL){
 	   temp=queue->quetopptr->next;
 	   free(queue->quetopptr);
        queue->quetopptr=temp;    
    }
